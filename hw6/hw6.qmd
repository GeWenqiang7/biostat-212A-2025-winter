...
In this time series forecasting task, different models exhibited varying levels of generalization ability $(CV\ R^2)$ and predictive performance (Test $R^2$).

**Baseline** relied solely on the previous day's trading volume, lacked generalization capability, and achieved a test $R^2$ of $0.35$, serving as a benchmark.

**AR(5)** used data from the past five days for predictions, showing moderate generalization ability ($CV\ R^2=0.34$), while its test $R^2=0.51$ indicated that it effectively captured short-term trends.

**Random Forest (RF)** performed similarly to AR(5) ($CV\ R^2=0.35$, Test $R^2=0.51$), likely due to its limited ability to fully leverage time series features, preventing it from significantly outperforming the linear model.

Meanwhile, **Boosting** demonstrated the best generalization ability ($CV\ R^2=0.38$) and the highest predictive performance (Test $R^2 = 0.54$), suggesting that it was able to better learn the dynamic patterns of trading volume and make more accurate forecasts.
...
# Perform PCA
pca_result <- prcomp(gene_t, scale. = TRUE)

# Extract PCA scores (first two principal components)
pca_scores <- as.data.frame(pca_result$x)

# Add labels for healthy/diseased samples
pca_scores$Group <- c(rep("Healthy", 20), rep("Diseased", 20))

# Plot PCA results
ggplot(pca_scores, aes(x = PC1, y = PC2, color = Group)) +
  geom_point(size = 3, alpha = 0.7) +
  labs(title = "PCA of Gene Expression", x = "Principal Component 1", y = "Principal Component 2") +
  theme_minimal()
